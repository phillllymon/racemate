...
module.exports.default = async function handler(req, res) {
    ...
    req.on("end", () => {
        try {
            ...
            const { userId, token, seriesId } = parsed;
                ...
                if (users.length === 1) {
                    bcrypt.compare(token, users[0]["login_token"]).then((tokenMatch) => {
                        if (tokenMatch) {
                            sql`
                                SELECT * FROM series
                                WHERE id = ${seriesId} AND owner = ${userId}
                            `.then((existingSeriesInfo) => {
                                if (existingSeriesInfo.length === 1) {
                                    ...
                                    res.writeHead(200, { "Content-Type": "application/json" });
                                    res.end(JSON.stringify({ message: "series not found or user not owner" }));
                                }
                            });
                        } else {
                            res.writeHead(200, { "Content-Type": "application/json" });
