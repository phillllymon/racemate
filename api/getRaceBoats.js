...
module.exports.default = async function handler(req, res) {
    ...
    req.on("end", async () => {
        try {
            const { userId, token, raceId } = JSON.parse(body);
            ...
            const rows = await sql`SELECT * FROM race_boats WHERE race_id = ${raceId} ORDER BY created_at ASC`;

            res.writeHead(200, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ message: "ok", rows }));
        } catch (err) {
